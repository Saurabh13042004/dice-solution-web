...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    if (isDark) {
      return vec4(buf[0].x * 0.8, buf[0].y * 0.9, buf[0].z * 1.2, 1.0);
    } else {
      return vec4(buf[0].x * 1.2, buf[0].y * 1.1, buf[0].z * 0.8, 1.0);
    }
  }
...
const CPPNShaderMaterial = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1), isDark: true },
  vertexShader,
  fragmentShader
);
...
function ShaderPlane({ isDark }: { isDark: boolean }) {
  ...
  useFrame((state) => {
    ...
    materialRef.current.isDark = isDark;
...
function ShaderBackground({ isDark }: { isDark: boolean }) {
  ...
  return (
    <div 
      ref={canvasRef} 
      className={`absolute inset-0 -z-10 w-full h-full transition-colors duration-500 ${
...
export default function Hero({
  ...
  badgeLabel = "New",
  ctaButtons = [
    { text: "Book Strategy Call", href: "#contact", primary: true },
    { text: "View Our Process", href: "#process" }
  ],
  microDetails = ["60‑day MVP", "AI‑powered workflow", "Startup‑focused"]
}: HeroProps) {
  ...
  const textColorMuted = isDark ? 'text-white/75' : 'text-black/75';
  const textColorFaded = isDark ? 'text-white/60' : 'text-black/60';
  const borderColor = isDark ? 'border-white/10' : 'border-black/10';
  const bgColor = isDark ? 'bg-white/5' : 'bg-black/5';
  const hoverBgColor = isDark ? 'hover:bg-white/20' : 'hover:bg-black/20';
  const hoverBgSecondary = isDark ? 'hover:bg-white/5' : 'hover:bg-black/5';
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div ref={badgeRef} className={`inline-flex items-center gap-2 rounded-full border ${borderColor} ${bgColor} px-3 py-1.5 backdrop-blur-sm`}>
          <span className={`text-[10px] font-light uppercase tracking-[0.08em] ${textColorFaded}`}>{badgeLabel}</span>
          <span className={`h-1 w-1 rounded-full ${isDark ? 'bg-white/40' : 'bg-black/40'}`} />
          <span className={`text-xs font-light tracking-tight ${textColorMuted}`}>{badgeText}</span>
        </div>

        <h1 ref={headerRef} className={`max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight ${textColor} sm:text-6xl md:text-7xl`}>
          {title}
        </h1>

        <p ref={paraRef} className={`max-w-xl text-left text-base font-light leading-relaxed tracking-tight ${textColorMuted} sm:text-lg`}>
          {description}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border ${borderColor} px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 ${isDark ? 'focus:ring-white/30' : 'focus:ring-black/30'} duration-300 ${
                button.primary
                  ? `${bgColor} ${textColor} backdrop-blur-sm ${hoverBgColor}`
                  : `${textColorMuted} ${hoverBgSecondary}`
              }`}
            >
              {button.text}
            </a>
        ...
        <ul ref={microRef} className={`mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight ${textColorFaded}`}>
          {microDetails.map((detail, index) => {
            const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
            return (
              <li key={index} ref={refMap[index]} className="flex items-center gap-2">
                <span className={`h-1 w-1 rounded-full ${isDark ? 'bg-white/40' : 'bg-black/40'}`} /> {detail}
              </li>
            );
          })}
        </ul>
      ...
      <div className={`pointer-events-none absolute inset-x-0 bottom-0 h-24 transition-colors duration-500 ${
